...
if calculate:
    try:
        ...
        def create_pdf(username):
            ...
            styles.add(ParagraphStyle(name='HeaderBlue', fontSize=16, textColor=colors.HexColor("#0d47a1"), alignment=1, spaceAfter=6))
            styles.add(ParagraphStyle(name='SubHeader', fontSize=11, textColor=colors.black, spaceAfter=6, fontName='Helvetica-Bold'))
            ...
            # ===== HEADER =====
            elements.append(Paragraph("<b>Freight Calculation Report</b>", styles['HeaderBlue']))
        
            elements.append(Paragraph(f"Generated by: {username}", styles['NormalSmall']))
            elements.append(Paragraph(f"Date: {datetime.now().strftime('%d %B %Y')}", styles['NormalSmall']))
            ...
            voyage_data = [
                ["Route", f"{port_pol} → {port_pod}"],
                ["Cargo", f"{qyt_cargo:,.0f} {type_cargo.split()[1]}"],
                ["Distance", f"{distance_pol_pod:,.0f} NM"],
                ["Voyage Duration", f"{total_voyage_days:.2f} Days"],
            ]
        
            for k, v in voyage_data:
                elements.append(Paragraph(f"{k} : <b>{v}</b>", styles['NormalSmall']))
        
            elements.append(Spacer(1, 12))
        
            # ===== VARIABLE COST =====
            var_data = [
                ["Fuel Cost", fmt_rp(cost_fuel)],
                ["FW Cost", fmt_rp(cost_fw)],
                ["Premi", fmt_rp(premi_cost)],
                ["Port Cost", fmt_rp(port_cost)],
            ]
        
            t_var = Table(var_data, colWidths=[5*cm, 4*cm])
            ...
            for k, v in owner_data.items():
                owner_table_data.append([k, fmt_rp(v)])
        
            t_owner = Table(owner_table_data, colWidths=[5*cm, 4*cm])
            ...
            # ===== COMBINE LEFT-RIGHT =====
            t_main = Table([[t_var, t_owner]], colWidths=[9*cm, 9*cm])
            t_main.setStyle(TableStyle([
                ('BOX', (0, 0), (-1, -1), 1.5, colors.black),
                ('INNERGRID', (0, 0), (-1, -1), 0.2, colors.lightgrey),
            ]))
        
            elements.append(Paragraph("Cost Breakdown", styles['SubHeader']))
            elements.append(t_main)
            elements.append(Spacer(1, 12))
        
            # ===== OPEX =====
            opex_data = [
                ["General Overhead", fmt_rp(total_general_overhead)],
                ["Depreciation", fmt_rp(depreciation_cost)],
                ["Other Cost", fmt_rp(other_cost)],
            ]
        
            t_opex = Table(opex_data, colWidths=[9*cm, 9*cm])
            ...
            elements.append(t_opex)
            elements.append(Spacer(1, 12))
        
            # ===== TOTAL =====
            total_data = [["TOTAL COST", fmt_rp(total_cost)]]
        
            t_total = Table(total_data, colWidths=[9*cm, 9*cm])
            ...
            if additional_breakdown:
                ...
                add_data = []
                for k, v in additional_breakdown.items():
                    add_data.append([k, fmt_rp(v)])
            
                t_add = Table(add_data, colWidths=[9*cm, 9*cm])
            
                t_add.setStyle(TableStyle([
                    ('BOX', (0, 0), (-1, -1), 1.5, colors.black),  # kotak luar tebal
                    ('LINEBELOW', (0, 0), (-1, -1), 0.2, colors.lightgrey),  # garis dalam tipis
                    ('ALIGN', (1, 0), (-1, -1), 'RIGHT'),
                ]))
            
                elements.append(Paragraph("Additional Cost", styles['SubHeader']))
